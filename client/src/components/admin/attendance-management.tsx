...
export function AttendanceManagement({ teachers }: AttendanceManagementProps) {
  ...
  const updateStudentMutation = useMutation({
    mutationFn: async ({ id, name, courseName }: { id: string; name: string; courseName: string }) => {
      return apiRequest("PATCH", `/api/admin/students/${id}`, { name, courseName });
    },
    onSuccess: () => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <Select value={selectedTeacherId} onValueChange={(v) => { setSelectedTeacherId(v); setSelectedStudentId(""); }} data-testid="select-teacher">
      ...
      {selectedTeacherId && studentList.length > 0 && (
        <Card>
              ...
              <Table>
                ...
                <TableBody>
                  {studentList.map(student => (
                    <TableRow key={student.id} className={student.id === selectedStudentId ? "bg-accent" : ""} data-testid={`row-student-${student.id}`}>
